...
def objective_function(X, y, w, lamb):
    ...
    summed[summed < 0] = 0.0 # Max against 0

    obj_value = (lamb/2.0)*np.sum(w.T*w.T) + (1.0/N)*np.sum(summed)
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        # Only want subset of all elements
        A_t_p_x = Xtrain[A_t,:]
        A_t_p_y = ytrain[A_t]

        # Only want index of elements of At+ where y*wt.T*xt < 1
        filtered = A_t_p_y*(w*A_t_p_x.T).sum(axis=0)
        filtered = filtered.flatten()
        mask = (filtered < 1.0)

        A_t_p_x = A_t_p_x[mask,:] 
        A_t_p_y = A_t_p_y[mask] 

        eta_t = 1.0/(lamb*iter)

        w_t_p_h = (1.0 - eta_t*lamb)*w + (eta_t/k)*(((A_t_p_y*A_t_p_x.T).T).sum(axis=0)).reshape(-1,1)

        w = min(1.0, (1/np.sqrt(lamb))/np.sqrt(np.sum(w_t_p_h.T*w_t_p_h.T)))*w_t_p_h
...
def pegasos_test(Xtest, ytest, w, t = 0.):
    ...
    XtestP = np.array(Xtest)
    ytestP = np.array(ytest)
    N = len(ytest)
    ...
    notSame = ytestP - predy
    notSame[notSame != 0.0] = 1.0


    test_acc = (N-np.sum(notSame))/N 

...
def pca(X = np.array([]), no_dims = 50):
    ...
    w, v = np.linalg.eig(cMatrix)

    # Grab indexes of eigenvectors to choose based on eigenvalues
    sIndex = w.argsort()[-no_dims:]

    # Grab the eigenvectors
    M = np.take(v, sIndex, axis=1)

    # Create matrix Y
    Y = np.einsum('ij,ki->kj',M,X)
...
def reconstruction_error(orig = np.array([]), decompressed = np.array([])):
    ...
    error = (1.0/N)*np.sum((orig-decompressed)**2)

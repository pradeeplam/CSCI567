...
def transform_image(image, code_vectors):
    ...
    r = np.zeros((x.shape[0],code_vectors.shape[0]))

    # match x w/ code vectors
    # Same stuff as done in kmeans
    a1 = ((x[:,None]-code_vectors)**2)
    a2 = np.sum(a1,axis=2)
    a3 = np.argmin(a2,axis=1)
    r[np.arange(a3.size),a3] = 1
    r = np.argmax(r, axis=1) # Convert from one hot to index

    x = np.take(code_vectors,r,axis=0)

    rows,pixels,rgb = image.shape

    return x.reshape(rows,pixels,rgb)
